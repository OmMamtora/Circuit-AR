...
// });
// ─────────────────────────────────────────────────────────────────────────
// Circuit AR — video player page (SHORTS STYLE)
// ─────────────────────────────────────────────────────────────────────────
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    if (visitBtn) {
        ...
        visitBtn.style.opacity = "0";
        visitBtn.style.visibility = "hidden";
        visitBtn.style.transition = "opacity 1s ease-in-out";
    }
    ...
    // YouTube Shorts Play/Pause Pop Animation
    let indicatorTimeout;
    const showIndicator = (icon) => {
        if (!playPauseIndicator) return;
        playPauseIndicator.textContent = icon;
    ...
    if (video) {
        ...
        if (unmuteBtn) {
            unmuteBtn.addEventListener("click", (e) => {
                e.stopPropagation(); // Stop tapArea from catching this click
                video.muted = false;
                unmuteBtn.hidden = true;
        ...
        // Handle Tap-to-Play/Pause anywhere on the screen
        if (tapArea) {
            tapArea.addEventListener("click", () => {
                // If they tap the screen while muted, just unmute them and hide the button
                if (unmuteBtn && !unmuteBtn.hidden) {
                    video.muted = false;
                    unmuteBtn.hidden = true;
                    if (video.paused) video.play();
                    return;

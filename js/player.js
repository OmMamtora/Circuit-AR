...
// Shared fallback video, used if a member doesn't have their own file yet.
const SHARED_PROMO_VIDEO = "assets/video/amit-ke-circuits-ar-promo.mp4";
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    if (video) {
        // Try the member's own video first (per-member file, or an explicit
        // videoUrl set in data.js). If that 404s/errors, we fall back to
        // the shared promo video below, so every member always shows *some*
        // video instead of a dead spinner.
        ...
        // If the member-specific video fails, fall back to the shared promo
        // video once. If THAT also fails, give up gracefully and show the
        // visit button anyway.
        video.addEventListener("error", () => {
            if (!triedFallback && video.src.indexOf(SHARED_PROMO_VIDEO) === -1) {
                triedFallback = true;
                video.src = SHARED_PROMO_VIDEO;
                video.load();
                tryPlay();
                return;
            }

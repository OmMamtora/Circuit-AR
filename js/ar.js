...
document.addEventListener("DOMContentLoaded", () => {
    ...
    // function getMemberId(decodedText){
    ...
    function onScanSuccess(decodedText){
        ...
        const memberId=getMemberId(decodedText);

        foundLabel.innerHTML="Opening AR...";

        html5QrCode.stop().then(()=>{

            setTimeout(()=>{

                // window.location.href=`player.html?id=${memberId}`;
                if(memberId){

                    window.location.href = `player.html?id=${encodeURIComponent(memberId)}`;
